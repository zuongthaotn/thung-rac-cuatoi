...
from AI.MACD.sources.broker import Broker
...
def training(genomes, config):
    ...
    for i in range(20, rows_number):
        ...
        for j in range(0, len(brokers)):
            ...
            decision = nets[j].activate((macd[i], signal[i], macd_h[i-1], macd_h[i-2]))  # decision can be a real number form 0 to 1
