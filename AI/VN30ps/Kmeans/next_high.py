...
def prepareData(htd):
    ...
    htd['Next_High'] = htd['High'].shift(-1)
    htd['Next_High_Is_Higher'] = htd.apply(
        lambda x: (1 if (x["Next_High"] > x["High"]) else 0), axis=1)
    htd['Next_High_Is_Lower'] = htd.apply(
        lambda x: (1 if (x["Next_High"] < x["High"]) else 0), axis=1)
...
# u = prepared_data.loc[prepared_data.Next_High_Is_Higher == 1]
# w = prepared_data.loc[prepared_data.Next_High_Is_Higher == 0]
...
k = prepared_data[["pass_2_return", "pass_1_return", "today_return", "EMA_H", "RSI_trend", "Next_High_Is_Higher"]]
X = k[["RSI_trend", "EMA_H", "pass_1_return", "today_return"]].to_numpy()
y = k.Next_High_Is_Higher.to_numpy()
...
yy = xxx.Next_High_Is_Lower.to_numpy()
...
# xxx.to_csv(str(current_folder)+'/VN30F2312_high.csv')
