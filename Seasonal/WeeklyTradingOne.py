...
prices = get_pricing(ticker_id, start_date=start_date, end_date=end_date, fields=['close'])
...
for weekday in buyAndSellDays:
    ...
    # f.write(history_log)
    # f.close()
    if total_wd_commission > 0:
        print("[" + ticker_id + "][" + str(_v.WEEK_DAY_SHORT[buyDay]) + " - " + str(_v.WEEK_DAY_SHORT[sellDay]) + "] Total commission of " + ticker_id + ": " + str(
            round(total_wd_commission * 1000, 2)) + ". Profits: " + str(time_profit) + ". Losses: " + str(time_loss))
    #
...
if best_profit > 0:
    print("[Blue Chip]" + ticker_id + " giving best profit: " + str(round(best_profit * 1000, 2)) + " when buy on " + best_buy_day + " and sell on " + best_sell_day)
else:
