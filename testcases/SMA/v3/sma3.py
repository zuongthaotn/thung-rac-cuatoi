...
class Sma01(Strategy):
    ...
    def init(self):
        ...
        self.maX = self.I(SMA, price, self.x)
    ...
    def next(self):
        try:
            ...
            #     self.position.close()
            #     self.buy_price = 0
            elif self.buy_price != 0 and (crossover(self.ma20, self.data.Close) or close_price < .89 * self.ma5[-1]) and self.d > 2:
...
stats = bt.run()
# print(stats)
# optimized_stats = bt.optimize(x=range(4, 22, 1))
